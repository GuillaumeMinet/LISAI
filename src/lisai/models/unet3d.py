...
from lisai.models.params import UNet3DParams
...
class UNet_PosEncod(nn.Module):
    def __init__(self, params: UNet3DParams):
        super().__init__()
        self.params = params
        ...
        if self.cab_skip_con:
            warnings.warn(
                "`cab_skip_con` is accepted but not implemented in 3D UNet_PosEncod yet; using plain skip connections instead.",
                stacklevel=2,
            )
        ...
        up_kernel = (1, self.upsampling_factor, self.upsampling_factor)
        up_stride = (1, self.upsampling_factor, self.upsampling_factor)
        if self.upsampling_method == "conv":
            self.upsamp_before = upsamp_block_3d(
                self.in_channels,
                self.in_channels,
                activation=self.activation,
        ...
        self.upsamp_after = upsamp_block_3d(
            self.feat,
            self.feat,
            activation=self.activation,
            kernel_size=up_kernel,
        ...
        self.upsamp_after_conv = conv_block_3d(
            self.feat,
            self.feat,
            activation=self.activation,
            norm=self.norm,
