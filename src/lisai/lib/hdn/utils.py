...
def extract_patches_paired(x,y,patch_size,num_patches):
...
def extract_patches(x,y,patch_size,num_patches):
    """Deterministically extract patches from paired array (x,y) of images
    or single array x if y is None. 
    Parameters
    ----------
    x: numpy array
        Array of images.
...
def plotProbabilityDistribution(signalBinIndex, histogram, gaussianMixtureNoiseModel, min_signal, max_signal, n_bin, device,
    ...
    """Plots probability distribution P(x|s) for a certain ground truth signal. 
       Predictions from both Histogram and GMM-based Noise models are displayed for comparison.
        Parameters
        ----------
        signalBinIndex: int or list
            index of signal bin or list of signal bins. Values go from 0 to number of bins (`n_bin`).
        ...
        max_columns: int (optional)
            max number of columns allowed the figure
        """
    if isinstance(signalBinIndex,int):
        signalBinIndex_list = [signalBinIndex]
    elif isinstance(signalBinIndex,list):
        signalBinIndex_list = signalBinIndex

    histBinSize=(max_signal-min_signal)/n_bin
    n_plots = len(signalBinIndex_list)
    n_columns = min(2 * math.ceil(math.sqrt(n_plots)), max_columns)
    n_rows = math.ceil(n_plots / (n_columns // 2))

    fig = plt.figure(figsize=(6*n_columns, 5*n_rows))
    for i,signalBinIndex in enumerate(signalBinIndex_list):
